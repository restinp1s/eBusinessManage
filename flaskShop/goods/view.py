from flaskShop.goods import goods,goods_api
...
def get_goods_list():
    ...
    goods_list = [gd.to_dict() for gd in goods]
...
class Goods(Resource):
    def post(self):
        try:
            ...
            goods =models.Goods(name = name,number = number,price = price,weight = weight,cid_one = cid_one,cid_two = cid_two,cid_three = cid_three,introduce = introduce)
...
def upload_img():
    ...
    if not img_file:
        return to_dict_msg(1022)
    if allow_img(img_file.filename):
        folder = current_app.config.get('SERVER_IMG_UPLOAD') #获取static/img文件夹路径
        ...
        return to_dict_msg(1023)
    
def allow_img(filename):
    return '.' in filename and filename.rsplit('.',1)[1] in current_app.config['ALLOW_IMG']
